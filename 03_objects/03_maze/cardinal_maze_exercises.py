class Room():
    ...
    def add_north_door(self, north_room, door_description, lock = None):
        self.north_door = Door(self, north_room, door_description, lock)
        north_room.south_door = self.north_door 

    def add_south_door(self, south_room, door_description, lock = None):
        self.south_door = Door(self, south_room, door_description, lock)
        south_room.north_door = self.south_door

    def add_east_door(self, east_room, door_description, lock = None):
        self.east_door = Door(self, east_room, door_description, lock)
        east_room.west_door = self.east_door
    
    def add_west_door(self, west_room, door_description, lock = None):
        self.west_door = Door(self, west_room, door_description, lock)
...
class Game():
    ...
    def print_room(self):
        ...
        door_count = room.count_doors()
        if door_count == 1:
            print("There is 1 door")
        else:
            print(f"There are {door_count} doors")

        if room.north_door != None:
            print(f"There is a {room.north_door.description} door to the (n)orth.")
        if room.east_door != None:
            print(f"There is a {room.east_door.description} door to the (e)ast.")
        if room.south_door != None:
            print(f"There is a {room.south_door.description} door to the (s)outh.")
        if room.west_door != None:
            print(f"There is a {room.west_door.description} door to the (w)est.")
